...
@section('main')
                ...
                <div class="card-body">
                    <form onsubmit="doSearch(event)" class="row gx-3 gy-2 align-items-center mb-3">
                        ...
                        <div class="col-auto">
                            ...
                            <a href="#" onclick="showCreateModal(event)" class="btn btn-success"><i
                                    class="fas fa-plus"></i>
    ...
    @include('admin.dashboard.user.admin-user.create')

    @include('admin.dashboard.user.admin-user.change-password')
...
@push('scripts')
    ...
    <script>
        ...
        const showCreateModal = (event) => {
            ...
            $modalElement.find('[name="nama_lengkap"]').val('');
        ...
        const editRow = (element, event) => {
            ...
            $modalElement.find('[name="nama_lengkap"]').val(json.nama_lengkap);
            $modalElement.find('[name="email"]').val(json.email);
            $modalElement.find('[name="username"]').val(json.username);
            $modalElement.find('[name="alamat"]').val(json.alamat);
         ...
         */
        const table = {
            baseURL: `{{ route('admin-user.tables') }}`,
            element: $('#table'),
            setLoadingState() {
            ...
            render(resultJson) {
                ...
                resultJson.forEach(row => {
                    ...
                    htmlRow.find('.td-name').text(row.nama_lengkap);
